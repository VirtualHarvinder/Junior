...
def configure_ngrok():
    ngrok_token = "2pGb4ZBgKXRQgjPDrfuGNoXIwYD_4a1visWno5cTq8kdyq91H"
    try:
...
def update_soul_txt(public_url):
    with open("jatin5.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in jatin5.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("jatin5.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated jatin5.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update jatin5.txt on VPS: {str(e)}")
